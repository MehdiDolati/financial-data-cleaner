...
| CompareDatasetsUseCase.cs | `Compare` | lines[118,119,121], branches at lines[116] | **test** | `isDifferent`-true path in AcceptedByAbsolute/Relative switch cases — reachable via public API |
| CompareDatasetsUseCase.cs | `BuildMissingRecord` | (TryGetValue-false branch) | **exclude** | `MissingFromCandidateTimestamps` are timestamps present in `benchmarkLookup` (both derived from `benchmarkCandles`), so `TryGetValue` always returns true; false branch is defense-in-depth (FR-014) |
| CompareDatasetsUseCase.cs | `BuildExtraRecord` | (TryGetValue-false branch) | **exclude** | `ExtraInCandidateTimestamps` are timestamps present in `candidateLookup` (both derived from `candidateCandles`), so `TryGetValue` always returns true; false branch is defense-in-depth (FR-014) |
| CompareDatasetsUseCase.cs | `GetFieldValue` | lines[257], branch at lines[250] | **test** | Internal method but visible to tests via `InternalsVisibleTo`; default throw is reachable by casting out-of-range `(OhlcvField)99` (spec Clarification Q1) |
| CompareDatasetsUseCase.cs | `BuildToleratedAggregate` | lines[269], branch at lines[267] | **exclude** | `TryGetValue`-false branch cannot fire: `BuildToleratedAggregate` is only called for fields from `configuration.Fields` that were already inserted into the counts dictionary by the preceding comparison loop |
...
| DatasetScore.cs | `.ctor` | lines[72], branch at lines[70] | **test** | Internal constructor + `[JsonConstructor]` visible to tests via `InternalsVisibleTo`; guard arms reachable by calling `new DatasetScore(...)` with invalid average/reason/category-count combinations (spec Clarification Q1) |
...
| FindingCatalog.cs | `RefOf` | branch at lines[451] | **exclude** | Only called on spool lines written by `AppendLineAsync` with format `"finding|..."`; the pipe separator is always present; the no-pipe fallback cannot be reached through any legal call path |
...
| MetricScore.cs | `.ctor` | lines[47,52,59], branches at lines[45,50,57] | **test** | Internal constructor + `[JsonConstructor]` visible to tests via `InternalsVisibleTo`; guard arms reachable by calling `new MetricScore(...)` with invalid state/value combinations (spec Clarification Q1) |
...
| ToleranceResolver.cs | `PowerOfTen` | (reachable negative-exponent arithmetic) | **test** | (T042) Corrected from whole-method **exclude**: `InferFractionalStep` calls `PowerOfTen(-maxPrecision)` with `maxPrecision >= 1` (after the `maxPrecision <= 0` early return), so the `1/10^n` loop IS reachable and is exercised by `InferFractionalStep_FiveDigitForex`/`_TwoDigitCrypto`/`_SixDecimalPlaces`/`_VeryHighPrecision`. The method is now branch-free negative-exponent arithmetic and is measured (present in `coverage.json`). Only the non-negative-exponent path is excludable — isolated in `PowerOfTenPositive` (below) per FR-005 |
| ToleranceResolver.cs | `PowerOfTenPositive` | (whole helper) | **exclude** | (T042) The isolated, genuinely-unreachable positive-exponent helper (`10^n` for `n >= 0`). No legal call supplies a non-negative exponent to the tolerance-inference logic (`InferFractionalStep` only computes `10^(-maxPrecision)`), so it is unreachable through any legal call. Excluded at the smallest scope (a single private helper, absent from `coverage.json`) and preserved rather than deleted per FR-014; revisit and test if a future caller makes a non-negative exponent reachable (E8) |
...
| ToleranceResolver.cs | `.cctor` (static ctor, in nested `Defaults` class) | lines[17,18,21,22] | **exclude** | Compiler-generated static constructor for `const decimal` fields in the nested `Defaults` holder — C# emits a `.cctor` for `const decimal` (unlike `const int`); values are inlined by the compiler. Isolated at smallest scope via nested class with `[ExcludeFromCodeCoverage]` |

### Classification summary

- **test**: 17 arms — reachable via public API, test-visible internal entry points, or out-of-range/undeclared-enum-cast values. These will be covered by new tests (T004–T007).
- **exclude**: 12 arms — genuinely unreachable through any legal call. These will receive `[ExcludeFromCodeCoverage(Justification=…)]` at the smallest scope (T010–T012).
...
All **test**-classified arms are reachable through the public API, via out-of-range enum casts, or through test-visible internal entry points. `GetFieldValue`, `MetricScore` constructor, and `DatasetScore` constructor are `internal` and require `InternalsVisibleTo` (already declared in `Validator.Application.csproj`) for test access. No additional `InternalsVisibleTo` entries are needed beyond what already exists.
